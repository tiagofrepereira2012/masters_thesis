...
Com o surgimento de pesquisas relacionadas a ataques de \textit{spoofing}, bases de dados utilizadas para validar métodos propostos foram construídas. Nesta Seção, as principais bases de dados serão descritas.
...
A base de dados Replay Attack \cite{ChingovskaBIOSIG2012} consiste de gravações de vídeos de curta duração ($\sim10s$) tanto de simulações de acessos reais quanto simulações de ataques em 50 identidades diferentes, utilizando um computador portátil. Ela contém 1.200 vídeos, sendo que 200 contemplam simulações de acessos reais e 1000 contemplam simulações de ataques em três cenários diferentes com duas diferentes condições de iluminação e suporte. Os cenários de ataque incluem as condições em que o atacante:
...
\begin{enumerate}
        ...
        \item \textbf{highdef}: Exibe fotografias e vídeos em alta resolução ($1024\times768$) em um iPad.
\end{enumerate}
...
\caption[Algumas capturas da base de dados Replay Attack]{Algumas capturas da base de dados Replay Attack (retirado de \cite{ChingovskaBIOSIG2012}).} \label{fig:Database}
\end{center}
\end{figure}
...
\end{equation}
onde $\tau$ é o limiar de detecção de ataques, $D$ é um subconjunto da base de dados, $FAR$ é a taxa de falsas aceitações e $FRR$ é a taxa de falsas rejeições. Neste protocolo, recomenda-se para o valor de $\tau$ a pontuação no ponto de $EER$ obtido no conjunto de calibração (devel set). O ponto de $EER$ denota a taxa de erro a um dado limiar onde $FRR$ e $FAR$ possuem o mesmo valor.
...
O objetivo desta base de dados é investigar a efetividade de diferentes tipos de ataques com as suas respectivas resoluções. Para isso, a base de dados possui um protocolo de avaliação composto de sete cenários.
...
\caption[Exemplos de acessos reais e ataques da base de dados CASIA FASD]{Exemplos de acessos reais e ataques da base de dados CASIA FASD (retirado de \cite{zhangface})} 
\label{fig:casia_db}
\end{center}
\end{figure}
...
Embora seja possível burlar um sistema de autenticação de face utilizando maquiagem, cirurgia plástica ou máscaras; fotografias e vídeos são provavelmente as ameaças mais comuns. Além disso, devido à crescente popularidade das redes sociais na WEB, (facebook, youtube, flickr, instagram e outros) uma grande quantidade de conteúdo multimídia, especialmente vídeos e fotografias, está disponível e estes dados podem ser utilizados facilmente para atacar um sistema de autenticação de faces. Para mitigar os sucessos dos ataques dessa natureza, contramedidas eficazes devem ser pesquisadas e desenvolvidas.
...
\begin{table}[ht]
\caption{Performance em termos de $HTER(\%)$ da contramedida proposta por (Chingovska et al. 2012) nas três principais bases de dados de referência.}
\begin{center}
...
Ainda analizando texturas, \cite{Pereira_LBP_2012} propuseram uma contramedida utilizando a dinâmica de uma textura ao longo do tempo utilizando o descritor $LBP-TOP$. Complementar ao descritor $LBP$, o descritor $LBP-TOP$ além de observar as componentes espaciais (direção X e Y), ele observa padrões de textura orientados no tempo (direção X e T e direção Y e T) como se pode observar na Figura \ref{img:LBPTOP}.
